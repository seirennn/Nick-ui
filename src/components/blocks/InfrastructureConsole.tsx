...
export function InfrastructureConsole() {
  ...
  const handleCopyKey = () => {
    navigator.clipboard.writeText('nick_live_sec_98f4bc819e0134');
    setCopiedKey(true);
    ...
    <div className="w-full rounded-2xl border border-border bg-card p-6 md:p-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-border">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Button variant="secondary" size="sm" onClick={handleCopyBlock} className="text-xs">
      ...
      {/* Primary Telemetry Gauges & Latency Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* CPU Utilization Dial */}
        <div className="p-4 rounded-xl bg-card border border-border flex flex-col items-center justify-between">
        ...
        <div className="p-4 rounded-xl bg-card border border-border flex flex-col justify-between">
          <div>
            ...
            <div className="mt-3 flex items-baseline gap-1">
              <span className="text-3xl font-semibold tracking-tight text-text-primary">12.4</span>
          ...
          <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
            ...
            <Sparkline data={[18, 16, 15, 14, 13, 12.4]} width={90} height={22} color="#10b981" />
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-card border border-border flex flex-col justify-between">
          <div>
            ...
            <div className="mt-3 flex items-baseline gap-1">
              <span className="text-3xl font-semibold tracking-tight text-text-primary">4.82</span>
          ...
          <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
            <span className="text-[11px] text-text-muted">Throughput</span>
            <Sparkline data={[3.8, 4.1, 4.3, 4.6, 4.7, 4.82]} width={90} height={22} color="var(--primary)" />
          </div>
        </div>
      </div>

      {/* Deployed Microservices Table */}
        ...
        <div className="rounded-xl border border-border overflow-hidden bg-card/60">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-secondary/40 border-b border-border/80 text-text-muted font-medium">
                <tr>
                  ...
                  <tr key={srv.name} className="hover:bg-secondary/30 transition-colors">
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="font-mono font-medium text-text-primary">{srv.name}</span>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <Badge variant="engraved" className="text-[10px] font-mono">
                        {srv.version}
                      </Badge>
                    </td>
                    <td className="py-3 px-4">
      ...
      {/* Production Overrides & Security Governance */}
        ...
        <div className="rounded-xl border border-border p-5 space-y-4 bg-card/60 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between">
              ...
              <Badge variant="engraved" className="text-[10px]">Read / Write</Badge>
            </div>
            <p className="text-metadata text-text-muted mt-1">
